...
static const char *TAG = "app_wifi_ui";

LV_FONT_DECLARE(font_alipuhui20);

// 全局变量改为静态，避免命名冲突
static lv_obj_t *wifi_scan_page = NULL;     // wifi扫描页面 obj
static lv_obj_t *wifi_connect_page = NULL;  // wifi连接页面 obj
static lv_obj_t *wifi_password_page = NULL; // wifi密码页面 obj
static lv_obj_t *wifi_list = NULL;          // wifi列表  list
static lv_obj_t *label_wifi_connect = NULL; // wifi连接页面label 
static lv_obj_t *ta_pass_text = NULL;       // 密码输入文本框 textarea
static lv_obj_t *roller_num = NULL;         // 数字roller
static lv_obj_t *roller_letter_low = NULL;  // 小写字母roller
static lv_obj_t *roller_letter_up = NULL;   // 大写字母roller
static lv_obj_t *label_wifi_name = NULL;    // wifi名称label
...
static void btn_back_to_main_cb(lv_event_t * e)
{
    ...
    if(code == LV_EVENT_CLICKED) {
        ESP_LOGI(TAG, "Back to main screen clicked");
        backToMS();
    }
}
...
static void lv_wifi_connect(void)
{
    ...
    // 添加返回主屏幕按钮
    lv_obj_t *btn_back = lv_btn_create(wifi_connect_page);
    lv_obj_set_size(btn_back, 80, 40);
    lv_obj_align(btn_back, LV_ALIGN_BOTTOM_MID, 0, -20);
    lv_obj_add_event_cb(btn_back, btn_back_to_main_cb, LV_EVENT_CLICKED, NULL);
    
    lv_obj_t *label_back = lv_label_create(btn_back);
    lv_label_set_text(label_back, "返回主屏");
...
static void btn_connect_cb(lv_event_t * e)
{
    ...
    if(code == LV_EVENT_CLICKED) {
        ...
        if(*wifi_password != '\0') // 判断是否为空字符串
        {
            ...
            strncpy(wifi_account.wifi_ssid, wifi_ssid, sizeof(wifi_account.wifi_ssid)-1);
            strncpy(wifi_account.wifi_password, wifi_password, sizeof(wifi_account.wifi_password)-1);
            ESP_LOGI(TAG, "connected to ap SSID:%s password:%s",
                    wifi_account.wifi_ssid, wifi_account.wifi_password);
            lv_wifi_connect(); // 显示wifi连接界面
            // 发送WiFi账号密码信息到队列
            if(xQueueWifiAccount) {
...
static void list_btn_cb(lv_event_t * e)
{
    ...
    lv_obj_add_event_cb(btn_back, btn_back_cb, LV_EVENT_CLICKED, NULL); // 添加按键处理函数
    ...
    lv_obj_add_event_cb(btn_connect, btn_connect_cb, LV_EVENT_CLICKED, NULL); // 事件处理函数
    ...
    lv_obj_add_event_cb(btn_del, btn_del_cb, LV_EVENT_CLICKED, NULL);  // 事件处理函数
    ...
    lv_obj_add_event_cb(btn_num_ok, btn_num_cb, LV_EVENT_CLICKED, NULL); // 事件处理函数
    ...
    lv_obj_add_event_cb(btn_letter_low_ok, btn_letter_low_cb, LV_EVENT_CLICKED, NULL); // 事件处理函数
    ...
    lv_obj_add_event_cb(btn_letter_up_ok, btn_letter_up_cb, LV_EVENT_CLICKED, NULL); // 事件处理函数
...
static void wifi_connect(void *arg)
{
    ...
    while (true)
    {
        ...
        if(xQueueReceive(xQueueWifiAccount, &wifi_account, portMAX_DELAY))
        {
            ...
            strncpy((char *)wifi_config.sta.ssid, wifi_account.wifi_ssid, sizeof(wifi_config.sta.ssid)-1);
            strncpy((char *)wifi_config.sta.password, wifi_account.wifi_password, sizeof(wifi_config.sta.password)-1);
...
void start_wifi_app(void)
{
    ...
    // 添加返回主屏幕按钮
    lv_obj_t *btn_back = lv_btn_create(wifi_scan_page);
    lv_obj_set_size(btn_back, 80, 40);
    lv_obj_align(btn_back, LV_ALIGN_BOTTOM_MID, 0, -20);
    lv_obj_add_event_cb(btn_back, btn_back_to_main_cb, LV_EVENT_CLICKED, NULL);
